import { Check, Download, FileText, Undo2 } from 'lucide-react'
import { useEffect, useMemo, useState } from 'react'
import { Link } from 'react-router-dom'
import { useBrokerAccount } from '../../../shared/lib/AccountContext'
import { getConsentLog } from '../../../shared/lib/onboarding/api'
import type { ConsentLogEntry } from '../../../shared/lib/onboarding/types'
...
import { printCustodyContract } from '../../custody/model/contract'
import { paidOutProfit } from '../../dashboard/model/dashboard-data'
import { printRegulatorReport, REGULATORS, type Regulator } from '../model/regulator-report'
...
export function DocumentsPage() {
  ...
  const [custody, setCustody] = useState<CustodyAccount[]>(loadCustodyAccounts)

  useEffect(() => {
    const sync = () => setCustody(loadCustodyAccounts())
  ...
  useEffect(() => {
    void getConsentLog().then(setConsentLog)
  }, [])
...
function renderRegulator(contracts: Contract[], invested: number, clientName: string, accountNumber: string) {
  ...
  const data = {
    ...
    invested,
    accrued,
    paidOut: paidOutProfit,
    contracts: contracts.map((contract) => ({
      id: contract.id,
      productName: contract.productName,
      amount: contract.amount,
      opened: human(contract.opened),
      rate: contract.rate,
    })),
  }

  return (
    <section className="rounded-[18px] border border-[var(--trigonum-border)] bg-white px-6 py-5 shadow-[0_8px_30px_rgb(8_27_58/8%)]">
      <h2 className="text-[15px] font-semibold text-[var(--trigonum-ink)]">Отчётность для регулятора</h2>
      <p className="mt-1 max-w-[70ch] text-xs text-[var(--trigonum-muted)]">
        Выписка по счёту в форме, принятой в вашей юрисдикции: сводные сведения, договоры, начисленный и выплаченный
        доход. Суммы приводятся в национальной валюте по курсу на отчётную дату.
      </p>
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        {(Object.keys(REGULATORS) as Regulator[]).map((key) => {
          const profile = REGULATORS[key]
          return (
            <button
              key={key}
              type="button"
              onClick={() => printRegulatorReport(key, data)}
              className="flex items-center justify-between gap-3 rounded-xl border border-[var(--trigonum-border)] px-4 py-3.5 text-left transition hover:border-[var(--trigonum-violet)]"
            >
              <span className="min-w-0">
                <span className="block text-sm font-bold text-[var(--trigonum-ink)]">
                  Подготовить отчёт · {profile.label}
                </span>
                <span className="mt-0.5 block text-[11px] text-[var(--trigonum-muted)]">
                  {profile.authority} · раз в {profile.period} · {profile.currency}
                </span>
              </span>
              <Download size={16} className="shrink-0 text-[var(--trigonum-violet)]" />
            </button>
          )
        })}
      </div>
...
/** Договоры счетов хранения: заявка и подпись — часть документооборота, а не настройка. */
function renderCustody(accounts: CustodyAccount[], clientName: string, brokerAccount: string) {
  if (accounts.length === 0) return null
  const signed = accounts.filter((account) => account.signedAt).length

  return (
    <section className="rounded-[18px] border border-[var(--trigonum-border)] bg-white px-6 py-5 shadow-[0_8px_30px_rgb(8_27_58/8%)]">
      <div className="flex flex-wrap items-baseline justify-between gap-3">
        <div>
          <h2 className="text-[15px] font-semibold text-[var(--trigonum-ink)]">Договоры счетов хранения</h2>
          <p className="mt-1 text-xs text-[var(--trigonum-muted)]">
            {accounts.length} договора · подписано {signed}
          </p>
        </div>
        <Link to="/deposit/custody" className="text-xs font-semibold text-[var(--trigonum-violet)]">
          Счета в Trigonum →
        </Link>
      </div>
      <div className="mt-4 flex flex-col gap-2.5">
        {accounts.map((account) => {
          ...
          return (
            <div
              key={account.id}
              className="grid items-center gap-3.5 rounded-xl border border-[var(--trigonum-border)] px-4 py-3.5 lg:grid-cols-[minmax(0,1.7fr)_minmax(0,1fr)_auto]"
            >
              <div className="flex min-w-0 items-center gap-3">
                <span className="grid size-[38px] shrink-0 place-items-center rounded-[10px] bg-[var(--trigonum-violet-soft)] text-[var(--trigonum-violet)]">
                  <FileText size={18} />
                </span>
                <div className="min-w-0">
                  <p className="text-sm font-bold text-[var(--trigonum-ink)]">
                    Счёт хранения · № {account.contractNumber}
                  </p>
                  <p className="mt-0.5 text-[11px] text-[var(--trigonum-muted)]">
                    {network.label} · {account.application.asset} · заявка от {human(account.createdAt)}
                  </p>
                </div>
              </div>
              <div>
                ...
                <p className="mt-1 text-[13px] font-semibold text-[var(--trigonum-ink)]">
                  {account.signedAt ? human(account.signedAt) : 'не подписан'}
                </p>
              </div>
              <div className="flex shrink-0 items-center gap-2">
                <span
                  className={`rounded-full px-2.5 py-1.5 text-[11px] font-bold ${
                    account.status === 'active'
                      ? 'bg-[#eef7f1] text-[var(--trigonum-success)]'
                      : 'bg-[#fdf6e8] text-[#92650c]'
                  }`}
                >
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1.5 rounded-lg border border-[var(--trigonum-border)] px-2.5 py-1.5 text-xs font-semibold text-[var(--trigonum-violet)]"
                >
                  <Download size={14} />
                  PDF
                </button>
...
function renderConsentLog(entries: ConsentLogEntry[]) {
  const active = new Map<string, ConsentLogEntry>()
  for (const entry of [...entries].reverse()) {
    if (entry.action === 'granted') active.set(entry.templateId, entry)
    else active.delete(entry.templateId)
  }
  ...
  return (
    <section className="rounded-[18px] border border-[var(--trigonum-border)] bg-white px-6 py-5 shadow-[0_8px_30px_rgb(8_27_58/8%)]">
      <div className="flex flex-wrap items-baseline justify-between gap-3">
        <div>
          <h2 className="text-[15px] font-semibold text-[var(--trigonum-ink)]">Журнал согласий</h2>
          <p className="mt-1 text-xs text-[var(--trigonum-muted)]">
            Кто и когда подписал документ, а что было отозвано
          </p>
        </div>
        <span className="text-[13px] font-bold tabular-nums text-[var(--trigonum-ink)]">
          Действует: {active.size}
        </span>
      </div>

      {entries.length === 0 ? (
        <p className="mt-4 text-[13px] text-[var(--trigonum-muted)]">
          Записей пока нет — согласия появятся здесь после подписания документов на шаге соглашений.
        </p>
      ) : (
        <div className="mt-4 flex flex-col divide-y divide-[var(--trigonum-border)]">
          {entries.map((entry) => (
            <div key={entry.id} className="flex flex-wrap items-center gap-3 py-3 first:pt-0 last:pb-0">
              <span
                className={`grid size-8 shrink-0 place-items-center rounded-lg ${
                  entry.action === 'granted'
                    ? 'bg-[color-mix(in_srgb,var(--trigonum-success)_12%,white)] text-[var(--trigonum-success)]'
                    : 'bg-[color-mix(in_srgb,var(--trigonum-danger)_10%,white)] text-[var(--trigonum-danger)]'
                }`}
              >
                {entry.action === 'granted' ? <Check size={15} /> : <Undo2 size={15} />}
              </span>

              <div className="min-w-0 flex-1">
                <p className="text-sm font-semibold text-[var(--trigonum-ink)]">
                  {entry.name} <span className="text-xs font-normal text-[var(--trigonum-muted)]">v{entry.version}</span>
                </p>
                <p className="text-[11px] text-[var(--trigonum-muted)]">
                  {entry.action === 'granted' ? 'Согласие подписано' : 'Согласие отозвано'}
                </p>
              </div>
