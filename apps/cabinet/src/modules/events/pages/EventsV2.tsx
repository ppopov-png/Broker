...
/** Комиссии Event: 1% при входе в сделку плюс доля от прибыли. */
function EventFees({ amount, gross }: { amount: number; gross: number }) {
  const schedule = FEE_SCHEDULES.event
  const fees = calcFees({ amount, grossProfit: gross, schedule })
    ...
    <div className="mt-3 border-t border-[var(--trigonum-border)] pt-3">
      ...
      <div className="mt-2 space-y-1.5">
        <div className="flex items-baseline justify-between gap-3">
          <span className="text-[11px] text-[var(--trigonum-muted)]">За управление · {schedule.managementOnDeposit}% при пополнении</span>
          <b className="text-xs font-bold tabular-nums text-[var(--trigonum-danger)]">− {formatCurrency(fees.management)}</b>
        </div>
        <div className="flex items-baseline justify-between gap-3">
          <span className="text-[11px] text-[var(--trigonum-muted)]">За результат · {schedule.resultShare}% от прибыли</span>
      ...
      <p className="mt-2 text-[10px] leading-[1.45] text-[var(--trigonum-muted)]">
        Управление удерживается один раз при входе в сделку. Комиссия за результат при убытке не взимается.
      </p>
    </div>
  )
}
