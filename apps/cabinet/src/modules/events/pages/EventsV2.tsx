...
function cover(seedValue: string) {
  ...
  const pairs = [
    ['#5a5ac4', '#12ccff'], ['#3f3f8a', '#af47ff'], ['#25254f', '#12ccff'],
    ['#7575ff', '#af47ff'], ['#161638', '#5a5ac4'], ['#3f3f8a', '#92f222'],
  ]
...
function urgency(event: LiveEvent) {
  if (full(event)) return { label: 'Заполнен', bg: '#eeeef8', color: '#5a5ac4', border: '#cdcdf0' }
  const share = Math.max(0, event.capacity - event.committed) / event.capacity
  if (event.secondsLeft <= 600 || share <= 0.06) return { label: 'Почти закрыт', bg: '#fdecec', color: '#b3383c', border: '#f6cdcd' }
  if (event.secondsLeft <= 1800 || share <= 0.15) return { label: 'Быстро заполняется', bg: '#fdf6e8', color: '#8a5f06', border: '#f2e2c2' }
  return { label: 'Открыт', bg: '#eef7f1', color: '#2e9e4f', border: '#e4e4f0' }
}
  ...
  return (
    <div className="relative shrink-0" style={{ width: size, height: size }}>
      <svg viewBox={`0 0 ${size} ${size}`} width={size} height={size} className="-rotate-90">
        ...
        <circle cx={size / 2} cy={size / 2} r={radius} fill="none" stroke="#7575ff" strokeWidth={stroke} strokeLinecap="round" strokeDasharray={dash} />
      </svg>
  ...
  return (
    <div className="mb-5 flex flex-wrap gap-2">
      {items.map((item) => {
        ...
        return <button key={item.id} type="button" onClick={() => onChange(item.id)} className={`inline-flex items-center gap-2 rounded-xl border px-3.5 py-2 text-sm font-semibold transition ${active ? 'border-[#25254f] bg-[#25254f] text-white' : 'border-[#e4e4f0] bg-white text-[#3a3a63] hover:border-[#5a5ac4]'}`}><Icon size={16} />{item.label}</button>
      })}
    </div>
  )
}
  ...
  return (
    <button type="button" onClick={onOpen} className="block w-full overflow-hidden rounded-[18px] border bg-white p-0 text-left shadow-[0_8px_30px_rgb(8_27_58/8%)] transition hover:-translate-y-0.5 hover:shadow-[0_14px_34px_rgb(8_27_58/12%)]" style={{ borderColor: status.border }}>
      ...
      <div className="p-[18px_20px]">
        <h3 className="text-[19px] font-bold text-[#25254f]">{event.title}</h3>
        <p className="mt-1 text-sm text-[#71719b]">{event.shortIdea}</p>
        <div className="mt-3.5 flex items-baseline justify-between rounded-xl border border-[#e4e4f0] px-3.5 py-3"><span className="text-[10px] font-bold uppercase tracking-[.08em] text-[#71719b]">Целевой результат</span><span className="text-[15px] font-bold text-[#2e9e4f]">+{event.targetLow}–{event.targetHigh}%</span></div>
        <div className="mt-3.5">
          <div className="mb-2 flex items-end justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[.08em] text-[#71719b]">{full(event) ? 'Распределено' : 'Доступно'}</p><p className={`mt-1 text-xl font-bold tabular-nums ${full(event) ? 'text-[#5a5ac4]' : remaining / event.capacity <= .12 ? 'text-[#b3383c]' : 'text-[#25254f]'}`}>{full(event) ? formatCurrency(event.capacity) : formatCurrency(remaining)}</p></div><p className="text-[11px] text-[#71719b]">{filled.toFixed(1)}% из {formatCurrency(event.capacity)}</p></div>
          <div className="h-1.5 overflow-hidden rounded-full bg-[#eeeef8]"><div className="h-full rounded-full bg-[#5a5ac4] transition-[width] duration-700" style={{ width: `${filled}%` }} /></div>
        </div>
        <div className="mt-4 grid grid-cols-3 gap-px overflow-hidden rounded-xl bg-[#e4e4f0]">
        ...
        <div className="mt-4 flex items-center justify-between border-t border-[#e4e4f0] pt-3.5"><span className="text-xs text-[#71719b]">{event.velocityPerMinute > 0 ? `Темп ${formatCurrency(event.velocityPerMinute)}/мин` : 'Набор закрыт'}</span><span className={`rounded-lg px-3 py-1.5 text-xs font-bold ${full(event) ? 'bg-[#eeeef8] text-[#5a5ac4]' : event.minInvestment <= BALANCE ? 'bg-[#eef7f1] text-[#2e9e4f]' : 'bg-[#f5f5fa] text-[#71719b]'}`}>{full(event) ? 'Заполнен' : event.minInvestment <= BALANCE ? 'Доступен' : 'Недостаточно средств'}</span></div>
      </div>
    </button>
  )
}
...
function MiniCell({ label, value }: { label: string; value: string }) {
  return <div className="bg-white px-3 py-2.5"><p className="text-[10px] uppercase tracking-[.08em] text-[#71719b]">{label}</p><p className="mt-1 text-sm font-bold tabular-nums text-[#25254f]">{value}</p></div>
}
...
function Gallery({ events, onSelect }: { events: LiveEvent[]; onSelect: (id: string) => void }) {
  ...
  return (
    <div className="space-y-5">
      <div className="rounded-[18px] border border-[#e4e4f0] bg-white p-5 shadow-[0_8px_30px_rgb(8_27_58/8%)]">
        <div className="flex flex-wrap items-center gap-3">
          <span className="inline-flex items-center gap-2 text-sm font-semibold text-[#25254f]"><Filter size={17} />Фильтры</span>
          <select value={sort} onChange={(e) => setSort(e.target.value as SortMode)} className="rounded-xl border border-[#e4e4f0] bg-white px-3 py-2 text-sm text-[#3a3a63]"><option value="scarcity">Сначала самые дефицитные</option><option value="time">Меньше всего времени</option><option value="capacity">Меньше всего доступно</option><option value="return">Выше целевой результат</option></select>
          <select value={risk} onChange={(e) => setRisk(e.target.value as 'Все' | Risk)} className="rounded-xl border border-[#e4e4f0] bg-white px-3 py-2 text-sm text-[#3a3a63]"><option value="Все">Любой риск</option><option value="Низкий">Низкий</option><option value="Умеренный">Умеренный</option><option value="Высокий">Высокий</option></select>
...
function FilterButton({ active, tone, onClick, children }: { active: boolean; tone: 'green' | 'red'; onClick: () => void; children: string }) {
  const cls = active ? tone === 'green' ? 'border-[#d8ecdf] bg-[#eef7f1] text-[#2e9e4f]' : 'border-[#f6cdcd] bg-[#fdecec] text-[#b3383c]' : 'border-[#e4e4f0] bg-white text-[#3a3a63]'
...
function EventDetail({ event, onBack, onInvest, onContra }: { event: LiveEvent; onBack: () => void; onInvest: (amount: number) => void; onContra: (amount: number) => void }) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between"><button type="button" onClick={onBack} className="inline-flex items-center gap-2 text-sm font-semibold text-[#5a5ac4]"><ArrowLeft size={16} />Все Events</button><button type="button" onClick={() => setWatching((v) => !v)} className={`rounded-lg border px-3 py-1.5 text-xs font-bold ${watching ? 'border-[#d8ecdf] bg-[#eef7f1] text-[#2e9e4f]' : 'border-[#e4e4f0] bg-white text-[#3a3a63]'}`}>{watching ? '✓ Отслеживается' : 'Наблюдать'}</button></div>
      <div className="overflow-hidden rounded-[18px] border border-[#e4e4f0] bg-white shadow-[0_8px_30px_rgb(8_27_58/8%)]">
        ...
        <div className="bg-[linear-gradient(120deg,#161638_0%,#25254f_46%,#3f3f8a_100%)] p-5 text-white">
          <div className="grid gap-4 lg:grid-cols-[1.5fr_.62fr_.58fr] lg:items-center"><div><div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-white/10 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[.08em] text-[#cfe9ff]">{event.category}</span><span className="text-[10px] font-semibold text-white/60">{event.id}</span></div><h2 className="mt-2.5 text-2xl font-bold">{event.title}</h2><p className="mt-1 text-sm text-white/70">{event.shortIdea}</p></div><div className="lg:border-l lg:border-white/10 lg:pl-4"><p className="text-[10px] font-bold uppercase tracking-[.08em] text-white/60">Позиция TAIS</p><p className="mt-1.5 text-[19px] font-bold text-[#92f222]">{event.taisPosition}</p><p className="mt-1 text-xs text-white/70">Цель +{event.targetLow}–{event.targetHigh}% · {event.horizon}</p></div><div className="lg:border-l lg:border-white/10 lg:pl-4 lg:text-right"><p className="text-[10px] font-bold uppercase tracking-[.08em] text-white/60">До закрытия входа</p><p className="mt-1.5 text-2xl font-bold tabular-nums">{countdown(event.secondsLeft)}</p><span className={`mt-1.5 inline-flex rounded-full px-2.5 py-1 text-[10px] font-bold ${isFull ? 'bg-[#7575ff]/25 text-[#dcdcff]' : event.secondsLeft <= 900 ? 'bg-[#e5484d]/25 text-[#ffd7d7]' : 'bg-[#92f222]/20 text-[#e0ffb8]'}`}>{isFull ? 'Основной объём заполнен' : `${fillPct.toFixed(1)}% распределено`}</span></div></div>
        </div>
        <div className="grid grid-cols-2 divide-x divide-[#e4e4f0] md:grid-cols-4"><TopStat label="Объём Event" value={formatCurrency(event.capacity)} /><TopStat label={isFull ? 'Распределено' : 'Осталось'} value={isFull ? formatCurrency(event.committed) : formatCurrency(remaining)} accent={remaining / event.capacity <= .12 ? 'danger' : isFull ? 'violet' : undefined} /><TopStat label="Минимум" value={formatCurrency(event.minInvestment)} /><TopStat label="Инвесторов" value={`${event.participants}`} /></div>
      </div>
      <div className="rounded-[18px] border border-[#e4e4f0] bg-white px-6 py-5 shadow-[0_8px_30px_rgb(8_27_58/8%)]"><p className="text-[10px] font-bold uppercase tracking-[.08em] text-[#71719b]">Как проходит Event</p><div className="mt-4 grid grid-cols-2 gap-4 md:grid-cols-4">{steps.map((step, index) => { const done = step.state === 'done'; const active = step.state === 'active'; return <div key={step.title} className="relative pr-4"><div className={`absolute left-4 right-0 top-[15px] h-0.5 ${index === steps.length - 1 ? 'bg-transparent' : step.state === 'future' ? 'bg-[#e4e4f0]' : 'bg-[#5a5ac4]'}`} /><div className={`relative grid h-8 w-8 place-items-center rounded-full border-2 text-xs font-bold ${done ? 'border-[#25254f] bg-[#25254f] text-white' : active ? 'border-[#5a5ac4] bg-white text-[#5a5ac4]' : 'border-[#e4e4f0] bg-[#f5f5fa] text-[#b0b0c8]'}`}>{index + 1}</div><p className={`mt-3 text-sm font-bold ${done ? 'text-[#25254f]' : active ? 'text-[#5a5ac4]' : 'text-[#b0b0c8]'}`}>{step.title}</p><p className="mt-1 text-xs leading-[1.45] text-[#71719b]">{step.meta}</p></div> })}</div></div>
      <div className="grid items-start gap-4 xl:grid-cols-[minmax(0,1fr)_410px]">
        <div className="space-y-4">
          <Panel><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-[.08em] text-[#5a5ac4]">Суть Event</p><h3 className="mt-1.5 text-lg font-bold text-[#25254f]">{event.driver}</h3></div><button type="button" onClick={() => setShowThesis((v) => !v)} className="rounded-lg border border-[#e4e4f0] bg-white px-3 py-1.5 text-xs font-bold text-[#3a3a63]">{showThesis ? 'Свернуть' : 'Почему TAIS?'}</button></div><p className="mt-2.5 text-sm text-[#71719b]">{event.shortIdea}</p>{showThesis && <div className="mt-3 rounded-xl border border-[#d8ecdf] bg-[#eef7f1] px-4 py-3 text-sm leading-[1.55] text-[#1f5c36]">{event.thesis}</div>}</Panel>
          <Panel><div className="flex items-baseline justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[.08em] text-[#71719b]">Актив события</p><h3 className="mt-1.5 text-lg font-bold text-[#25254f]">{chart.symbol}</h3></div><span className={`rounded-lg px-2.5 py-1.5 text-xs font-bold ${side === 'tais' ? 'bg-[#eef7f1] text-[#2e9e4f]' : 'bg-[#f4e9ff] text-[#8321d6]'}`}>{event.taisPosition}</span></div><div className="mt-3.5"><EventMarketChart symbol={chart.symbol} tone={chart.tone} seed={event.id} height={280} /></div><p className="mt-2 text-[11px] text-[#71719b]">График: Bybit market data · React/Recharts.</p></Panel>
          <Panel><div className="grid gap-3 sm:grid-cols-3"><MetricBox label="Риск" value={event.risk} hint={`Горизонт ${event.horizon}`} /><MetricBox label="Дефицит" value={`${event.scarcity}/100`} hint="Доступность основного allocation" danger={event.scarcity >= 90} /><MetricBox label="Капитал Trigonum" value={`${Math.round(event.trigonumCapital / event.capacity * 100)}%`} hint={`${formatCurrency(event.trigonumCapital)} внутри Event`} success /></div><div className="mt-3 rounded-xl border border-[#e4e4f0] p-4"><div className="flex items-center gap-5"><div className="relative h-[108px] w-[108px] shrink-0"><GaugeRing fill={fillPct} trigonum={event.trigonumCapital / event.capacity * 100} /></div><div className="min-w-0 flex-1"><p className="inline-flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-[.08em] text-[#71719b]"><Gauge size={14} />Набор капитала</p><p className="mt-2 text-xl font-bold tabular-nums text-[#25254f]">{formatCurrency(event.committed)} из {formatCurrency(event.capacity)}</p><div className="mt-3.5 h-1.5 overflow-hidden rounded-full bg-[#eeeef8]"><div className="h-full rounded-full bg-[#5a5ac4]" style={{ width: `${fillPct}%` }} /></div><div className="mt-2.5 flex gap-4 text-[11px] text-[#71719b]"><span className="inline-flex items-center gap-1.5"><i className="h-2 w-2 rounded-full bg-[#5a5ac4]" />Капитал инвесторов</span><span className="inline-flex items-center gap-1.5"><i className="h-2 w-2 rounded-full bg-[#92f222]" />Доля Trigonum</span></div></div></div><div className="mt-3.5 grid grid-cols-3 gap-2"><MiniMetric label="Инвесторов" value={`${event.participants}`} /><MiniMetric label="Темп" value={event.velocityPerMinute > 0 ? `${formatCurrency(event.velocityPerMinute)}/мин` : 'Набор закрыт'} success={event.velocityPerMinute > 0} /><MiniMetric label="До заполнения" value={!isFull && event.velocityPerMinute > 0 ? `≈ ${Math.max(1, Math.ceil(minutesToFull))} мин` : 'Заполнен'} /></div></div></Panel>
        </div>
        <div className="xl:sticky xl:top-20">
          <div className="overflow-hidden rounded-[18px] border border-[#e4e4f0] bg-white shadow-[0_8px_30px_rgb(8_27_58/8%)]">
            <div className="border-b border-[#e4e4f0] p-4"><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-2.5"><Wallet size={18} className={side === 'tais' ? 'text-[#2e9e4f]' : 'text-[#8321d6]'} /><div><p className="text-[10px] font-bold uppercase tracking-[.08em] text-[#71719b]">Действие</p><h3 className="mt-1 text-base font-bold text-[#25254f]">{blocked ? 'Event заполнен' : side === 'tais' ? 'Инвестировать в Event' : 'Ставка против TAIS'}</h3></div></div><span className="text-xs font-bold tabular-nums text-[#71719b]">{countdown(event.secondsLeft)}</span></div><div className="mt-3 grid grid-cols-2 gap-2 rounded-xl bg-[#f5f5fa] p-1.5"><SideButton active={side === 'tais'} title="С TAIS" position={event.taisPosition} tone="green" onClick={() => setSide('tais')} /><SideButton active={side === 'contra'} title="Обратная" position={event.counterPosition} tone="violet" onClick={() => setSide('contra')} /></div></div>
            <div className="p-4">{blocked ? <div className="space-y-3"><div className="rounded-xl border border-[#cdcdf0] bg-[#eeeef8] p-4"><p className="text-[10px] font-bold uppercase tracking-[.08em] text-[#5a5ac4]">Основной Event заполнен</p><p className="mt-2 text-[17px] font-bold text-[#25254f]">Новые вложения больше не принимаются</p><p className="mt-2 text-sm leading-[1.45] text-[#3a3a63]">Капитал участников уже зафиксирован в Event до его завершения.</p></div><button type="button" onClick={() => setSide('contra')} className="w-full rounded-xl border border-[#e5cdf7] bg-[#f4e9ff] px-4 py-3 text-sm font-bold text-[#8321d6]">Рассмотреть ставку против TAIS</button></div> : <div className="space-y-4"><div><p className="text-[10px] font-bold uppercase tracking-[.08em] text-[#71719b]">Выбранная позиция</p><div className="mt-1.5 flex items-center justify-between"><p className={`text-[22px] font-bold ${side === 'tais' ? 'text-[#2e9e4f]' : 'text-[#8321d6]'}`}>{side === 'tais' ? event.taisPosition : event.counterPosition}</p><span className={`rounded-full px-2.5 py-1 text-[10px] font-bold ${side === 'tais' ? 'bg-[#eef7f1] text-[#2e9e4f]' : 'bg-[#f4e9ff] text-[#8321d6]'}`}>{side === 'tais' ? 'TAIS' : 'Обратная'}</span></div></div><AmountPicker amount={amount} setAmount={setAmount} min={event.minInvestment} max={maxAmount} quick={quick} />{side === 'tais' ? <div className="rounded-xl border border-[#e4e4f0] p-3.5"><p className="text-[10px] font-bold uppercase tracking-[.08em] text-[#71719b]">Сценарии на горизонте Event</p><Scenario label={`Осторожный · +${event.targetLow}%`} value={`+${formatCurrency(amount * event.targetLow / 100)}`} width={event.targetLow / event.targetHigh * 100} fill="linear-gradient(90deg,#5a5ac4,#7575ff)" color="#5a5ac4" /><Scenario label={`Целевой · +${event.targetHigh}%`} value={`+${formatCurrency(amount * event.targetHigh / 100)}`} width={100} fill="linear-gradient(90deg,#12ccff,#92f222)" color="#2e9e4f" /></div> : <div className="space-y-2"><div className="rounded-xl border border-[#e5cdf7] bg-[#f4e9ff] p-3"><p className="text-[10px] font-bold uppercase tracking-[.08em] text-[#8321d6]">Ставка против гипотезы TAIS</p><p className="mt-1.5 text-sm font-bold text-[#25254f]">TAIS не достигнет +{event.targetLow}% в горизонте {event.horizon}</p><p className="mt-1.5 text-xs leading-[1.45] text-[#8321d6]">Если нижняя граница цели TAIS не выполнена к завершению Event, ставка выигрывает.</p></div><div className="grid grid-cols-3 gap-2"><MiniMetric label="Коэффициент" value={`×${odds.toFixed(2)}`} violet /><MiniMetric label="Выплата" value={formatCurrency(amount * odds)} success /><MiniMetric label="Прибыль" value={`+${formatCurrency(amount * (odds - 1))}`} success /></div></div>}<button type="button" onClick={reserve} disabled={side === 'tais' ? !canTais : !canContra} className={`w-full rounded-xl px-4 py-3.5 text-sm font-bold text-white disabled:opacity-40 ${side === 'tais' ? 'bg-[#2e9e4f]' : 'bg-[#8321d6]'}`}>{side === 'tais' ? (remaining < amount ? 'Такой объём уже недоступен' : `Открыть ${event.taisPosition}`) : 'Поставить на неуспех TAIS'}</button></div>}{(myAllocation > 0 || myContra > 0) && <div className="mt-4 border-t border-[#e4e4f0] pt-4"><p className="text-[10px] font-bold uppercase tracking-[.08em] text-[#71719b]">Ваши позиции</p><div className="mt-2 space-y-2">{myAllocation > 0 && <PositionRow label={event.taisPosition} amount={myAllocation} tone="green" />}{myContra > 0 && <PositionRow label={event.counterPosition} amount={myContra} tone="violet" />}</div><p className="mt-2.5 text-[11px] text-[#71719b]">Капитал зафиксирован до завершения Event</p></div>}</div>
          </div>
        </div>
      </div>
    </div>
...
function Panel({ children }: { children: React.ReactNode }) {
  return <div className="rounded-[18px] border border-[#e4e4f0] bg-white p-5 shadow-[0_8px_30px_rgb(8_27_58/8%)]">{children}</div>
}

function TopStat({ label, value, accent }: { label: string; value: string; accent?: 'danger' | 'violet' }) {
  return <div className="p-3.5"><p className="text-[10px] font-semibold uppercase tracking-[.08em] text-[#71719b]">{label}</p><p className={`mt-1 text-base font-bold tabular-nums ${accent === 'danger' ? 'text-[#b3383c]' : accent === 'violet' ? 'text-[#5a5ac4]' : 'text-[#25254f]'}`}>{value}</p></div>
}

function MetricBox({ label, value, hint, danger, success }: { label: string; value: string; hint: string; danger?: boolean; success?: boolean }) {
  return <div className="rounded-xl bg-[#f5f5fa] p-3"><p className="text-[10px] font-semibold uppercase tracking-[.08em] text-[#71719b]">{label}</p><p className={`mt-1 text-base font-bold ${danger ? 'text-[#b3383c]' : success ? 'text-[#2e9e4f]' : 'text-[#25254f]'}`}>{value}</p><p className="mt-2 text-xs text-[#71719b]">{hint}</p></div>
}

function GaugeRing({ fill, trigonum }: { fill: number; trigonum: number }) {
  const c1 = 2 * Math.PI * 45
  const c2 = 2 * Math.PI * 31
  return <><svg viewBox="0 0 108 108" className="h-full w-full -rotate-90"><circle cx="54" cy="54" r="45" fill="none" stroke="#eeeef8" strokeWidth="11" /><circle cx="54" cy="54" r="45" fill="none" stroke="#5a5ac4" strokeWidth="11" strokeLinecap="round" strokeDasharray={`${fill / 100 * c1} ${c1}`} /><circle cx="54" cy="54" r="31" fill="none" stroke="#92f222" strokeWidth="4" strokeLinecap="round" strokeDasharray={`${trigonum / 100 * c2} ${c2}`} /></svg><div className="absolute inset-0 flex flex-col items-center justify-center"><span className="text-xl font-bold tabular-nums text-[#25254f]">{fill.toFixed(1)}%</span><span className="mt-0.5 text-[9px] font-bold uppercase tracking-[.1em] text-[#71719b]">набрано</span></div></>
}

function MiniMetric({ label, value, success, violet }: { label: string; value: string; success?: boolean; violet?: boolean }) {
  return <div className={`rounded-lg p-2.5 ${violet ? 'border border-[#e5cdf7] bg-[#f4e9ff]' : 'bg-[#f5f5fa]'}`}><p className="text-[10px] font-semibold uppercase tracking-[.08em] text-[#71719b]">{label}</p><p className={`mt-1 text-[15px] font-bold tabular-nums ${success ? 'text-[#2e9e4f]' : violet ? 'text-[#8321d6]' : 'text-[#25254f]'}`}>{value}</p></div>
}
...
function SideButton({ active, title, position, tone, onClick }: { active: boolean; title: string; position: string; tone: 'green' | 'violet'; onClick: () => void }) {
  return <button type="button" onClick={onClick} className={`rounded-lg border px-3 py-2.5 text-left ${active ? tone === 'green' ? 'border-[#a9dcb9] bg-white' : 'border-[#e5cdf7] bg-white' : 'border-transparent bg-transparent'}`}><span className={`block text-[10px] font-bold uppercase tracking-[.08em] ${tone === 'green' ? 'text-[#2e9e4f]' : 'text-[#8321d6]'}`}>{title}</span><span className="mt-0.5 block text-[13px] font-bold text-[#25254f]">{position}</span></button>
}
...
function AmountPicker({ amount, setAmount, min, max, quick }: { amount: number; setAmount: (value: number) => void; min: number; max: number; quick: number[] }) {
  return <div className="space-y-3"><div className="flex items-end gap-3"><label className="min-w-0 flex-1"><span className="text-[10px] font-bold uppercase tracking-[.08em] text-[#71719b]">Сумма</span><span className="mt-1 flex items-center rounded-xl border border-[#e4e4f0] bg-white px-3"><span className="text-sm font-bold text-[#71719b]">$</span><input type="number" min={min} max={max} value={amount} onChange={(e) => setAmount(Number(e.target.value))} className="w-full bg-transparent px-2 py-2.5 text-xl font-bold text-[#25254f] outline-none" /></span></label><div className="shrink-0 pb-0.5 text-right"><p className="text-[10px] uppercase tracking-[.08em] text-[#71719b]">Доступно</p><p className="mt-1 text-sm font-bold text-[#25254f]">{formatCurrency(BALANCE)}</p></div></div><input type="range" min={min} max={max} value={amount} onChange={(e) => setAmount(Number(e.target.value))} className="w-full accent-[#5a5ac4]" /><div className="flex flex-wrap gap-1.5">{quick.map((value) => <button key={value} type="button" onClick={() => setAmount(value)} className={`rounded-lg border px-2.5 py-1.5 text-xs font-bold ${amount === value ? 'border-[#5a5ac4] bg-[#eeeef8] text-[#5a5ac4]' : 'border-[#e4e4f0] bg-white text-[#3a3a63]'}`}>{value === max ? 'MAX' : formatCurrency(value)}</button>)}</div></div>
}

function Scenario({ label, value, width, fill, color }: { label: string; value: string; width: number; fill: string; color: string }) {
  return <div className="mt-2.5"><div className="flex items-baseline justify-between"><span className="text-xs text-[#71719b]">{label}</span><span className="text-[13px] font-bold tabular-nums" style={{ color }}>{value}</span></div><div className="mt-1 h-2 rounded-full bg-[#f0f0f7]"><div className="h-full rounded-full" style={{ width: `${width}%`, background: fill }} /></div></div>
}

function PositionRow({ label, amount, tone }: { label: string; amount: number; tone: 'green' | 'violet' }) {
  return <div className={`flex items-center justify-between rounded-xl border px-3 py-2 ${tone === 'green' ? 'border-[#d8ecdf] bg-[#eef7f1]' : 'border-[#e5cdf7] bg-[#f4e9ff]'}`}><span className={`text-xs font-bold ${tone === 'green' ? 'text-[#2e9e4f]' : 'text-[#8321d6]'}`}>{label}</span><b className="text-sm tabular-nums text-[#25254f]">{formatCurrency(amount)}</b></div>
}
...
function SeasonView({ onOpen }: { onOpen: (event: PastEvent) => void }) {
  ...
  return <div className="space-y-5"><div className="overflow-hidden rounded-[18px] border border-[#e4e4f0] shadow-[0_8px_30px_rgb(8_27_58/8%)]"><div className="bg-[linear-gradient(120deg,#161638,#25254f_52%,#3f3f8a)] p-6 text-white"><p className="text-xs font-bold uppercase tracking-[.2em] text-[#92f222]">Архив TAIS Events</p><h2 className="mt-2 text-[28px] font-bold">Сезон IV · III квартал 2026</h2><p className="mt-2 max-w-[720px] text-sm leading-[1.5] text-white/75">Завершённые возможности: как быстро распределялся капитал, сколько инвесторов участвовало и каким был итог.</p></div></div><div className="grid gap-3 sm:grid-cols-3"><SummaryCard label="Распределено капитала" value={formatCurrency(capital)} /><SummaryCard label="Итог инвесторов" value={`+${formatCurrency(pnl)}`} success /><SummaryCard label="Прибыльных Event" value={`${profitable} из ${pastEvents.length}`} /></div><div className="grid gap-4 xl:grid-cols-2">{pastEvents.map((event) => <PastCard key={event.id} event={event} onOpen={() => onOpen(event)} />)}</div></div>
}

function SummaryCard({ label, value, success }: { label: string; value: string; success?: boolean }) {
  return <div className={`rounded-xl border p-3.5 ${success ? 'border-[#d8ecdf] bg-[#eef7f1]' : 'border-[#e4e4f0] bg-white'}`}><p className="text-[11px] font-semibold uppercase tracking-[.08em] text-[#71719b]">{label}</p><p className={`mt-1.5 text-xl font-bold tabular-nums ${success ? 'text-[#2e9e4f]' : 'text-[#25254f]'}`}>{value}</p></div>
}
...
function PastCard({ event, onOpen }: { event: PastEvent; onOpen: () => void }) {
  ...
  return <button type="button" onClick={onOpen} className="block w-full overflow-hidden rounded-[18px] border border-[#e4e4f0] bg-white p-0 text-left transition hover:-translate-y-0.5 hover:shadow-[0_12px_30px_rgb(8_27_58/10%)]"><div className="relative h-28 overflow-hidden" style={{ background: c.bg }}><div className="absolute inset-0 bg-[repeating-radial-gradient(circle_at_86%_140%,rgb(255_255_255/9%)_0_1px,transparent_1px_26px)]" /><div className="absolute -right-[26px] -top-[54px] h-[190px] w-[190px] rounded-full opacity-50" style={{ background: `radial-gradient(circle, ${c.accent} 0%, transparent 68%)` }} /><p className="absolute -bottom-3.5 left-4 text-[76px] font-semibold tracking-[-.04em] text-white/10">{symbolFor(event.position)}</p><div className="absolute inset-0 flex items-start justify-between p-[14px_18px] text-white"><span className="rounded-full bg-white/20 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[.08em]">{event.category}</span><span className="text-[11px] font-semibold text-white/80">{event.closedDate}</span></div></div><div className="p-[18px]"><div className="flex items-start justify-between gap-3"><div><h3 className="text-lg font-bold text-[#25254f]">{event.title}</h3><p className="mt-1 text-sm text-[#71719b]">{event.shortIdea}</p></div><span className={`shrink-0 rounded-xl px-3 py-2 text-[13px] font-bold ${short ? 'bg-[#fdecec] text-[#b3383c]' : 'bg-[#eef7f1] text-[#2e9e4f]'}`}>{event.position}</span></div><div className="mt-4 grid grid-cols-4 gap-2"><MetricCell label="Результат" value={`${event.result > 0 ? '+' : ''}${event.result}%`} good={event.result >= 0} /><MetricCell label="Капитал" value={formatCurrency(event.invested)} /><MetricCell label="Инвесторов" value={`${event.investors}`} /><MetricCell label="Заполнен за" value={event.fillTime} /></div><div className="mt-4 flex items-center justify-between border-t border-[#e4e4f0] pt-3.5"><div><p className="text-[10px] uppercase tracking-[.08em] text-[#71719b]">Итог инвесторов</p><p className={`mt-1 text-[17px] font-bold tabular-nums ${event.totalPnl >= 0 ? 'text-[#2e9e4f]' : 'text-[#b3383c]'}`}>{event.totalPnl > 0 ? '+' : ''}{formatCurrency(event.totalPnl)}</p></div><span className="rounded-lg bg-[#f5f5fa] px-2.5 py-1.5 text-xs font-semibold text-[#71719b]">Длительность {event.activeDuration}</span></div></div></button>
}

function MetricCell({ label, value, good }: { label: string; value: string; good?: boolean }) {
  return <div className="rounded-xl bg-[#f5f5fa] p-3"><p className="text-[10px] uppercase tracking-[.08em] text-[#71719b]">{label}</p><p className={`mt-1 text-sm font-bold tabular-nums ${good === true ? 'text-[#2e9e4f]' : good === false ? 'text-[#b3383c]' : 'text-[#25254f]'}`}>{value}</p></div>
}
...
function LedgerView({ onOpen }: { onOpen: (event: PastEvent, reference: number) => void }) {
  ...
  return <div className="grid items-start gap-5 lg:grid-cols-2"><Panel><h2 className="text-[15px] font-semibold text-[#25254f]">Упущенные возможности</h2><p className="mt-1 text-xs text-[#71719b]">Нажмите на строку — откроются детали закрытого Event</p><div className="mt-4 space-y-2.5">{rows.map(({ ref, event, value }) => { const w = Math.abs(value) / maxAbs * 50; return <button key={event.id} type="button" onClick={() => onOpen(event, ref.reference)} className="grid w-full grid-cols-[62px_1fr_128px] items-center gap-3 rounded-xl border border-transparent px-2 py-1.5 text-left hover:border-[#e4e4f0] hover:bg-[#f7f7fc]"><b className="text-sm text-[#25254f]">#{event.id.slice(-3)}</b><div className="relative h-[30px] min-w-[140px] rounded-lg bg-[#f7f7fc]"><div className="absolute bottom-0 left-1/2 top-0 w-px bg-[#d3d3e6]" /><div className="absolute top-[9px] h-3 rounded-full" style={{ background: value >= 0 ? '#5a5ac4' : '#e5484d', left: value >= 0 ? '50%' : `${50 - w}%`, width: `${w}%` }} /></div><div className="text-right"><p className={`text-[15px] font-bold tabular-nums ${value >= 0 ? 'text-[#2e9e4f]' : 'text-[#b3383c]'}`}>{value > 0 ? '+' : ''}{formatCurrency(value)}</p><p className="mt-0.5 text-[11px] text-[#71719b]">{event.result > 0 ? '+' : ''}{event.result}% · из {formatCurrency(ref.reference)}</p></div></button> })}</div></Panel><Panel><h2 className="text-[15px] font-semibold text-[#25254f]">Как читать эту таблицу</h2><p className="mt-2.5 text-sm leading-[1.55] text-[#71719b]">Условный расчёт: результат завершённого Event, применённый к сумме, которую вы могли бы вложить. Это не доход и не обязательство — только историческая иллюстрация.</p></Panel></div>
}
...
function HallView() {
  ...
  const backgrounds: Record<number, string> = { 1: 'linear-gradient(160deg,#5a5ac4,#25254f)', 2: 'linear-gradient(160deg,#3f3f8a,#25254f)', 3: 'linear-gradient(160deg,#25254f,#161638)' }
  return <div className="space-y-5"><div className="overflow-hidden rounded-[18px] shadow-[0_8px_30px_rgb(8_27_58/8%)]"><div className="bg-[linear-gradient(120deg,#161638,#3f3f8a_55%,#8321d6)] p-6 text-white"><p className="text-xs font-bold uppercase tracking-[.2em] text-[#e5cdf7]">Анонимный зал результатов</p><h2 className="mt-2 text-[28px] font-bold">Рекорды Events</h2></div></div><Panel><div className="flex items-baseline justify-between"><h2 className="text-[15px] font-semibold text-[#25254f]">Топ-3 по доходу инвесторов</h2><span className="rounded-full bg-[#eeeef8] px-2.5 py-1 text-[10px] font-bold text-[#5a5ac4]">Сезон IV</span></div><div className="mt-5 grid grid-cols-3 items-end gap-4">{ordered.map((item) => <div key={item.rank}><p className="text-center text-[11px] font-bold uppercase tracking-[.08em] text-[#71719b]">{item.alias}</p><p className="mt-1.5 text-center text-[26px] font-semibold tracking-[-.03em] text-[#2e9e4f]">+{formatCurrency(item.amount)}</p><p className="mb-3 mt-1 text-center text-xs text-[#71719b]">{item.meta}</p><div className="relative overflow-hidden rounded-t-xl" style={{ height: heights[item.rank], background: backgrounds[item.rank] }}><div className="absolute inset-0 bg-[repeating-radial-gradient(circle_at_50%_160%,rgb(255_255_255/10%)_0_1px,transparent_1px_22px)]" /><p className="absolute inset-x-0 top-3.5 text-center text-[11px] font-bold text-white/75">{item.rank === 1 ? 'Лидер сезона' : `${Math.round(item.amount / max * 100)}% от лидера`}</p><p className="absolute inset-x-0 bottom-3 text-center text-[34px] font-semibold text-white/60">{item.rank}</p></div></div>)}</div><div className="h-1.5 bg-[#25254f]" /></Panel><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">{hallData.map((item) => <Panel key={item.label}><p className="text-[11px] font-semibold uppercase tracking-[.08em] text-[#71719b]">{item.label}</p><p className="mt-3.5 text-[30px] font-semibold tracking-[-.03em] tabular-nums text-[#25254f]">{item.value}</p><p className="mt-2 text-xs text-[#71719b]">{item.meta}</p></Panel>)}</div></div>
}
...
function HistoryView({ onOpen }: { onOpen: (event: PastEvent, mine: { invested: number; season: string }) => void }) {
  ...
  return <div className="space-y-5"><Panel><h2 className="text-[19px] font-bold text-[#25254f]">История участия</h2><p className="mt-1.5 text-sm text-[#71719b]">Events, в которых вы участвовали. Нажмите на карточку — откроются детали события.</p><div className="mt-4 grid grid-cols-2 gap-px overflow-hidden rounded-xl border border-[#e4e4f0] bg-[#e4e4f0] md:grid-cols-3 xl:grid-cols-6">{stats.map(([label, value]) => <div key={label} className="bg-white p-3.5"><p className="text-[10px] font-semibold uppercase tracking-[.08em] text-[#71719b]">{label}</p><p className={`mt-1.5 text-lg font-bold tabular-nums ${label === 'Заработано' || label === 'Лучший Event' ? 'text-[#2e9e4f]' : 'text-[#25254f]'}`}>{value}</p></div>)}</div></Panel><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{rows.map((row) => { const c = cover(row.event.id); return <button key={row.event.id} type="button" onClick={() => onOpen(row.event, { invested: row.invested, season: row.season })} className="block w-full overflow-hidden rounded-[18px] border border-[#e4e4f0] bg-white p-0 text-left shadow-[0_8px_30px_rgb(8_27_58/8%)] transition hover:-translate-y-0.5 hover:shadow-[0_14px_34px_rgb(8_27_58/12%)]"><div className="relative h-[104px] overflow-hidden" style={{ background: c.bg }}><div className="absolute inset-0 bg-[repeating-radial-gradient(circle_at_86%_140%,rgb(255_255_255/9%)_0_1px,transparent_1px_24px)]" /><div className="absolute -right-6 -top-[50px] h-[170px] w-[170px] rounded-full opacity-50" style={{ background: `radial-gradient(circle, ${c.accent} 0%, transparent 68%)` }} /><p className="absolute -bottom-3 left-3.5 text-[68px] font-semibold text-white/10">{symbolFor(row.event.position)}</p><div className="absolute inset-0 flex items-start justify-between p-3.5 text-white"><span className="text-[11px] font-bold uppercase tracking-[.08em]">{row.season}</span><span className="rounded-full bg-white/20 px-2.5 py-1 text-[10px] font-bold">Участие</span></div></div><div className="p-4"><div className="flex items-baseline justify-between"><p className={`text-2xl font-bold tabular-nums ${row.earned >= 0 ? 'text-[#2e9e4f]' : 'text-[#b3383c]'}`}>{row.earned > 0 ? '+' : ''}{formatCurrency(row.earned)}</p><p className={`text-sm font-bold ${row.event.result >= 0 ? 'text-[#2e9e4f]' : 'text-[#b3383c]'}`}>{row.event.result > 0 ? '+' : ''}{row.event.result}%</p></div><p className="mt-1.5 text-[13px] font-semibold text-[#25254f]">{row.event.position}</p><div className="mt-3 flex items-baseline justify-between border-t border-[#e4e4f0] pt-3 text-xs text-[#71719b]"><span>Вложено {formatCurrency(row.invested)}</span><span>Event #{row.event.id.slice(-3)}</span></div></div></button> })}</div></div>
}
...
function ArchiveModal({ event, reference, mine, onClose }: { event: PastEvent; reference?: number; mine?: { invested: number; season: string }; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-[90] flex items-center justify-center bg-[rgb(8_27_58/45%)] p-8 backdrop-blur-[2px]" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose() }}><div className="max-h-[88vh] w-full max-w-[760px] overflow-auto rounded-[18px] bg-white shadow-[0_25px_50px_-12px_rgb(8_27_58/40%)]"><div className="relative overflow-hidden bg-[linear-gradient(160deg,#25254f_0%,#161638_100%)] px-6 py-5 text-white"><div className="absolute -right-[60px] -top-20 h-[260px] w-[260px] rounded-full bg-[radial-gradient(circle,rgb(117_117_255/40%)_0%,transparent_66%)]" /><p className="absolute -bottom-[34px] right-5 text-[108px] font-semibold text-white/[.07]">{symbolFor(event.position)}</p><div className="relative flex items-start justify-between gap-4"><div><div className="flex flex-wrap items-center gap-2"><span className="rounded-full border border-white/20 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[.08em] text-white/80">{event.category}</span><span className="text-[11px] font-bold uppercase tracking-[.12em] text-[#7575ff]">{reference ? `Упущенная возможность #${event.id.slice(-3)} · ${event.id}` : mine ? `Ваше участие · ${event.id}` : `Архив · ${event.id}`}</span></div><h2 className="mt-3 text-2xl font-bold">{event.title}</h2><p className="mt-1.5 text-[13px] text-white/70">{event.position} · завершён {event.closedDate}</p></div><button type="button" onClick={onClose} className="rounded-lg border border-white/25 bg-transparent p-2 text-white"><X size={16} /></button></div></div><div className="p-5"><div className="grid grid-cols-2 gap-px overflow-hidden rounded-xl border border-[#e4e4f0] bg-[#e4e4f0] md:grid-cols-4"><ModalMetric label="Результат Event" value={`${event.result > 0 ? '+' : ''}${event.result}%`} color={event.result >= 0 ? '#2e9e4f' : '#b3383c'} /><ModalMetric label="Итог инвесторов" value={`${event.totalPnl > 0 ? '+' : ''}${formatCurrency(event.totalPnl)}`} /><ModalMetric label="Заполнен за" value={event.fillTime} /><ModalMetric label={reference ? 'Ваш условный итог' : mine ? 'Ваш результат' : 'Инвесторов'} value={reference ? `${hypothetical > 0 ? '+' : ''}${formatCurrency(hypothetical)}` : mine ? `${mineEarned > 0 ? '+' : ''}${formatCurrency(mineEarned)}` : `${event.investors}`} color={reference ? hypothetical >= 0 ? '#2e9e4f' : '#b3383c' : mine ? mineEarned >= 0 ? '#2e9e4f' : '#b3383c' : undefined} /></div><div className="mt-4 rounded-xl border border-[#e4e4f0] p-3.5"><EventMarketChart symbol={chart.symbol} tone={chart.tone} seed={event.id} height={200} interval="240" limit={600} eventStart={start} eventEnd={end} /><p className="mt-2 text-[11px] text-[#71719b]">Период Event на графике выделен фоном.</p></div><div className="mt-4 grid gap-3 md:grid-cols-2"><div className="rounded-xl border border-[#e4e4f0] p-4"><p className="text-[10px] font-bold uppercase tracking-[.08em] text-[#71719b]">Исполнение</p><div className="mt-3 space-y-2 text-sm"><Row label="Инвесторов" value={`${event.investors}`} /><Row label="Заполнение" value={event.fillTime} /><Row label="Длительность" value={event.activeDuration} /></div></div>{event.totalPnl > 0 && event.bestPnl > 0 ? <div className="rounded-xl border border-[#e4e4f0] p-4"><p className="text-[10px] font-bold uppercase tracking-[.08em] text-[#71719b]">Топ-3 инвесторов Event</p><div className="mt-4 grid grid-cols-3 items-end gap-2">{[.74,1,.51].map((k, index) => <div key={k}><p className="text-center text-sm font-bold text-[#2e9e4f]">+{formatCurrency(Math.round(event.bestPnl * k))}</p><p className="my-1.5 text-center text-[10px] text-[#71719b]">{index === 1 ? `Участник ${event.bestAlias}` : `Участник #${event.id.slice(-3)}${index === 0 ? 'B' : 'C'}`}</p><div className="relative rounded-t-lg bg-[linear-gradient(160deg,#5a5ac4,#25254f)]" style={{ height: index === 1 ? 118 : index === 0 ? 84 : 64 }}><p className="absolute inset-x-0 bottom-2 text-center text-xl font-semibold text-white/60">{index === 1 ? 1 : index === 0 ? 2 : 3}</p></div></div>)}</div><div className="h-1 bg-[#25254f]" /></div> : <div className="rounded-xl border border-[#f6cdcd] bg-[#fdecec] p-4"><p className="text-[10px] font-bold uppercase tracking-[.08em] text-[#b3383c]">Итог Event</p><p className="mt-2 text-[19px] font-bold text-[#b3383c]">{formatCurrency(event.totalPnl)} · {event.result}%</p><p className="mt-2 text-xs leading-[1.45] text-[#8f2b2e]">Гипотеза TAIS не подтвердилась, прибыли по этому Event не было.</p></div>}</div><div className="mt-3 rounded-xl border border-[#e4e4f0] p-4"><p className="mb-3 text-[10px] font-bold uppercase tracking-[.08em] text-[#71719b]">Как проходил Event</p>{event.timeline.map((item, index) => <div key={`${item.time}-${index}`} className="grid grid-cols-[66px_26px_1fr] gap-3"><span className="pt-1 text-xs font-bold tabular-nums text-[#71719b]">{item.time}</span><div className="relative flex justify-center"><span className="relative z-10 grid h-[26px] w-[26px] place-items-center rounded-full border-2 border-[#5a5ac4] bg-[#eeeef8] text-[11px] font-bold text-[#5a5ac4]">{index + 1}</span>{index < event.timeline.length - 1 && <span className="absolute bottom-[-6px] top-[26px] w-0.5 bg-[#e4e4f0]" />}</div><div className="pb-4 pt-0.5"><p className="text-sm font-semibold text-[#25254f]">{item.label}</p><p className="mt-1 text-xs leading-[1.5] text-[#71719b]">{item.detail}</p></div></div>)}</div>{reference && <p className="mt-3 text-xs leading-[1.5] text-[#71719b]">Условный итог рассчитан от суммы {formatCurrency(reference)} по фактическому результату Event {event.result > 0 ? '+' : ''}{event.result}%. Это историческая иллюстрация, а не доход.</p>}{mine && <p className="mt-3 text-xs text-[#71719b]">Ваше участие: Event #{event.id.slice(-3)}, {mine.season}.</p>}</div></div></div>
}

function ModalMetric({ label, value, color }: { label: string; value: string; color?: string }) {
  return <div className="bg-white p-3.5"><p className="text-[10px] font-semibold uppercase tracking-[.08em] text-[#71719b]">{label}</p><p className="mt-1.5 text-[17px] font-bold tabular-nums" style={{ color: color ?? '#25254f' }}>{value}</p></div>
}

function Row({ label, value }: { label: string; value: string }) {
  return <div className="flex justify-between"><span>{label}</span><b className="text-[#25254f]">{value}</b></div>
}
