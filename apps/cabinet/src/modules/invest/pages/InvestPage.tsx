...
import { calcFees, FEE_SCHEDULES, feeLabel, type FeeFamily } from '../../../shared/lib/fees'
...
type Product = {
  ...
  risk: string
  description: string
  /** Пусто у бессрочного продукта: выбирать нечего. */
  terms: number[]
  /** Earn не имеет срока — договор бессрочный, тело можно вывести в окно вывода. */
  openEnded?: boolean
  payout: string
  payoutMonths: number
  min: number
  topup: string
  /** Когда можно забрать тело: у Earn — окно раз в неделю, у стратегий — конец срока. */
  withdraw: string
  /** Схема комиссий продукта. */
  feeFamily: FeeFamily
  tags: string[]
  headerBg: string
  accent: string
...
/** На сколько месяцев вперёд показываем доход по бессрочному продукту. */
const PROJECTION_MONTHS = 12

/** Ближайший понедельник — окно вывода тела по бессрочному договору. */
function nextWithdrawWindow(from = new Date()): Date {
  const date = new Date(from)
  date.setDate(date.getDate() + ((8 - date.getDay()) % 7 || 7))
  return date
}

/** Выплату дохода можно запрашивать не чаще раза в месяц. */
function payoutAvailableFrom(lastRequest: string | null): Date | null {
  if (!lastRequest) return null
  const next = new Date(lastRequest)
  next.setMonth(next.getMonth() + 1)
  return next > new Date() ? next : null
}

const PAYOUT_REQUESTS_KEY = 'trigonum-earn-payout-requests-v1'

function loadPayoutRequests(): Record<string, string> {
  try {
    const raw = window.localStorage.getItem(PAYOUT_REQUESTS_KEY)
...
const PRODUCTS: Product[] = [
  {
    ...
    risk: 'Низкий риск', description: 'Бессрочное размещение: заводите в любой момент, начисление идёт каждый день.', terms: [], openEnded: true, payout: 'по запросу, раз в месяц', payoutMonths: 1, min: 1000, topup: 'в любой момент', withdraw: 'раз в неделю', feeFamily: 'earn', tags: ['Без срока', 'Начисление каждый день'],
...
/** Короткая дата для окон вывода и выплат. */
...
export function InvestPage() {
  ...
  const [payoutRequests, setPayoutRequests] = useState<Record<string, string>>(loadPayoutRequests)
  ...
  const requestPayout = (contract: Contract) => {
    const next = { ...payoutRequests, [contract.id]: new Date().toISOString() }
    setPayoutRequests(next)
    try {
      window.localStorage.setItem(PAYOUT_REQUESTS_KEY, JSON.stringify(next))
    } catch {
  ...
  const form = useMemo(() => {
    ...
    // У бессрочного продукта срока нет, поэтому доход показываем проекцией на год.
    const horizon = selectedProduct.openEnded ? PROJECTION_MONTHS : selectedProduct.terms[termIndex] ?? selectedProduct.terms[0]
    const termMonths = topupContract && !selectedProduct.openEnded ? topupContract.termMonths : horizon
    ...
    // Комиссии считаем от того же валового дохода, что показан выше:
  ...
  const submit = () => {
    ...
    window.setTimeout(() => {
      ...
      if (topupContract) {
        ...
        const ends = selectedProduct.openEnded
          ? null
        ...
        const next: Contract = {
          ...
          termMonths: selectedProduct.openEnded ? 0 : form.termMonths,
          opened: '2026-09-04',
          ends,
          status: 'Активен',
...
function PayoutButton({ contract, blockedUntil, onRequest }: { contract: Contract; blockedUntil: Date | null; onRequest: () => void }) {
  ...
  return (
    <button
      type="button"
      onClick={onRequest}
      disabled={contract.income <= 0}
      className="rounded-lg border border-[var(--trigonum-border)] px-2.5 py-2 text-xs font-semibold text-[var(--trigonum-success)] transition hover:border-[var(--trigonum-success)] disabled:cursor-not-allowed disabled:opacity-45"
    >
      Запросить выплату
    </button>
  )
...
/**
 * Разбивка «до комиссий → на руки». Показываем всегда, в том числе у Earn,
 * где комиссия уже в ставке: клиент должен видеть, что она есть, а не
 * обнаружить её в договоре.
 */
function FeeBlock({ product, fees, months }: { product: Product; fees: ReturnType<typeof calcFees>; months: number }) {
  const schedule = FEE_SCHEDULES[product.feeFamily]

  if (schedule.netOfManagement) {
    return (
  ...
  return (
    <div className="rounded-xl border border-[var(--trigonum-border)] p-3.5">
      <p className="text-[11px] font-bold uppercase tracking-[.08em] text-[var(--trigonum-muted)]">
        Комиссии за {months} мес.
      </p>
...
function FeeRow({ label, value, negative = false, hint }: { label: string; value: string; negative?: boolean; hint?: string }) {
  return (
    <div className="flex items-baseline justify-between gap-3">
      <span className="text-xs text-[var(--trigonum-muted)]">
        {label}
        {hint && <span className="mt-0.5 block text-[10px] text-[var(--trigonum-muted)]">{hint}</span>}
      </span>
      <b className={`shrink-0 text-[13px] font-bold tabular-nums ${negative ? 'text-[var(--trigonum-danger)]' : 'text-[var(--trigonum-ink)]'}`}>
        {value}
      </b>
    </div>
  )
