...
export function ComplianceSection() {
  ...
  return (
    <section className="compliance-section" id="compliance">
      <header className="section-head"><h2>{compliance.title}</h2><p>{compliance.subtitle}</p></header>
      <div className="compliance-grid">
        {compliance.points.map((point, index) => {
          const Icon = icons[index] ?? ShieldCheck
          return <article key={point.title}><Icon strokeWidth={1.7} /><h3>{point.title}</h3><p>{point.text}</p></article>
        })}
      </div>
      <a className="compliance-link" href={REGULATOR_URL} target="_blank" rel="noreferrer">{compliance.link}<ExternalLink size={14} /></a>
    </section>
  )
}
