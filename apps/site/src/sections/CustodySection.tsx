...
/**
 * Блок, которого нет у большинства площадок, и именно его ищут те, кто уже
 * терял деньги: где физически лежат средства и на каком основании.
 */
  ...
  return (
    <section className="custody-section" id="custody">
      <div className="custody-intro">
        <span className="custody-badge">
          <Landmark size={18} strokeWidth={1.8} />
        </span>
        <h2>{custody.title}</h2>
        <p>{custody.subtitle}</p>
        <a className="custody-link" href={onboardingUrl()}>
          {custody.cta}
