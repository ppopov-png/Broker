...
/** Сколько занимает заявка целиком — из блока «как начать зарабатывать». */
const APPLICATION_SECONDS = 25 * 60

/** Центы здесь важны: без них счётчик стоит на месте. Разделитель — точка,
 *  как принято в долларах, при этом разряды остаются в стиле остальной страницы. */
function money(value: number): string {
  const cents = Math.round(value * 100)
  return `${usd(Math.trunc(cents / 100))}.${String(Math.abs(cents) % 100).padStart(2, '0')}`
}

/** Самый доходный план — на нём считаем «сколько бы вы заработали, пока читаете». */
const bestPlan = GOAL_PLANS.reduce((best, plan) => (plan.netAnnual > best.netAnnual ? plan : best))

/**
 * Обратный калькулятор: человек приходит с суммой, а не с депозитом. Показать
 * «сколько нужно завести» честнее, чем «сколько вырастет», — второе легко
    ...
    <div className="goal-calc">
      <header className="goal-calc-head">
        ...
        <label className="goal-input">
          ...
          <input
            ...
            step={10_000}
            value={goal}
            onChange={(event) => setGoal(Math.max(1000, Number(event.target.value) || 0))}
          />
        </label>
      </header>
      ...
      <div className="goal-controls">
        <div className="goal-presets" role="group" aria-label={calculator.goalLabel}>
          {PRESETS.map((preset) => (
            <button
              key={preset}
              type="button"
              className={goal === preset ? 'active' : undefined}
        ...
        <label className="goal-years">
          <span>
            {calculator.yearsLabel}: <b>{years}</b> {calculator.yearsUnit(years)}
          </span>
          <input
            type="range"
            min={1}
            ...
            {GOAL_PLANS.map((plan) => {
              ...
              return (
                <tr key={plan.id}>
                  ...
                  <td data-label={calculator.columns.net}>
                    <b>{pct(plan.netAnnual)}</b>
...
/**
 * Живой счётчик. Личная цифра за 25 минут анкеты слишком мала, чтобы
 * тикать на глазах, поэтому в реальном времени растёт результат всей
 * платформы, а личная проекция стоит рядом строкой — обе цифры настоящие.
 */
function GoalTicker({ goal }: { goal: number }) {
  ...
  const text = calculator.tickerText
    .replace('{goal}', usd(goal))
    .replace('{plan}', bestPlan.name)
    .replace('{app}', money(goalPerSecond * APPLICATION_SECONDS))
    .replace('{month}', money((goal * (bestPlan.netAnnual / 100)) / 12))

  return (
    <div className="goal-ticker">
      <Sparkles size={16} />
      <div>
        <span className="goal-ticker-label">{calculator.tickerTitle}</span>
        <strong>+{money(platformPerSecond * elapsed)}</strong>
        <p>
          <span className="goal-ticker-caption">{calculator.tickerCaption}</span>
          {text}
        </p>
      </div>
